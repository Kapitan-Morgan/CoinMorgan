...
function FindOnPage(name, status) {
	...
	if(input.length>=1)
	{
		function FindOnPageGo() {
			...
			for(var i=0; i<result.length;i++) {
				result_arr[i] = result[i].replace(eval(search), '<span class="each" style="background-color:yellow;">'+input+'</span>'); //находим нужные элементы, задаем стиль и сохраняем в новый массив
			}
			for(var i=0; i<result.length;i++) {
	...
	if(!status) { FindOnPageBack();$(".each").toggleClass('clicked');; } //Снимаем выделение
}
$(".button_for_search").click(function() {
	var top = $(".each").offset().top;
	console.log(top)
	$("html,body").scrollTop(top-350);
	top = 0
})
